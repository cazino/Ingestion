...
$(document).ready(function(){
  ...
  */
    ...
    data: "p=jax_cart_add2&type="+ objetType +"&id=" + trackId,
     success: function(xml){
      ...
      if($(xml).read("msg")){
        ...
        if($(xml).read("err_tra_alb") != "0"){msg_err = "Ce titre appartient à un album déjà présent dans votre panier";}//err_tra_alb
        if($(xml).read("err_alb_tra") != "0"){msg_err = "Cet album possède un titre déjà présent dans votre panier";}//err_alb_tra
  ...
  /***
   * Checkout :: Suppr. an item à partir du checkout
   * 061008-121235   
   */         
  ...
  data: "p=jax_cart_suppr_an_item&type="+ p_type +"&id=" + p_id+"&value="+p_cartid,
    ...
    if(p_type == "2"){
      // ré-activation du bt addalbum complet
  ...
  $("#add_1_"  + p_id).attr("disabled",false).removeClass("track_isbought").addClass("bt-cart-track");
    ...
    /**** TOTAL **
     *input : id de l'input où se trouve la chaine a calculer
     * ope : '+' ou '-' 
     * */
  ...
  $.fn.olcart_2_newcart = function(){
    ...
    */    
    ...
    // if(o.val() == "&nbsp;"){ ** tl-add_1|
      ...
      o.css({"color":"#fff","background":"#000"}).html("item ajouté");
      }
    ...
    1.--
        ...
        if(p_action_type == "+"){
          ...
          $("#a_"+$(this).read("cart_album_id") + "").find(".bt-alb-buy").toggleClass("alb-isbought").attr("onclick","").html("Album Acheté");
          tl_disabled($($(this)).read("cart_track_album_id:eq(0)"));// premier album ajouté au cart
          }else{
    ...
    if(o.is("a.bt-alb-buy")){
        ...
        tl_disabled($(xml).read("cart_track_album_id:eq(0)"));// premier album ajouté au cart
